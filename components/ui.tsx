...
export function StepBar({ current }: { current: number }) {
  const items = ["Signal", "Trend", "Human Insight", "Brand Opportunity", "Creative Route", "Campaign", "Assets"];
...
export function AiBadge() { return <span className="inline-flex items-center gap-1 rounded-full border border-black/15 bg-white/60 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider"><Sparkles size={11}/> AI synthesis</span> }
